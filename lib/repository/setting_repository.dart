...
Future<SettingData> initSettings() async {
  try {
    ...
    response = await dio.get(Urls.settings,
        options: d.Options(contentType: "application/json",
    ...
    appState.defaultCurrencyDecimalDigits = int.parse(_setting.setting!["default_currency_decimal_digits"]);
    ...
    appState.shippingCharge = double.parse( _setting.setting!["shipping_charge"]);
    appState.minimumOrderValue = double.parse(_setting.setting!["minimum_order_value"]);
...
Future<Map<String, dynamic>>  getKeysLists(String languageCode) async {
  try {
    d.Response response;
    response = await dio.get(Urls.keysLists, queryParameters: {"lang_code" : languageCode});
    print(response);
    if (response.statusCode == 200) {
    return response.data['data'].isEmpty ? {} : response.data['data'];
  } else {
